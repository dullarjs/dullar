...
export default class VField extends mixins(Queen).with(Props) {
    ...
    if (this.clearable || this.iconName) {
      ...
      icon.push(
        withDirectives(h(VFlexItem,
          ...
          {
            default: () => [h(VIcon, { name, size: "16", cursor: "pointer" }, { default: () => [] })]
          }
        ), [[vShow, this.showIcon]]
      ));
    }
  ...
  createInput(): VNode[] {
    ...
    if (VALID_TYPE.indexOf(this.type) > -1) {
      if (this.type === "textarea") {
        area.push(
          h(VFlexItem,
            { class: ["v-field-textarea", columnDisplayClass], flex: 1 },
            {
              default: () => [
                h(
                  "textarea",
                ...
                withDirectives(h(
                  "div",
                  {
                    class: ["v-field-textarea-counter"],
                  },
                  {
                    default: () => [
        ...
        area.push(
          h(VFlexItem,
            { class: ["v-field-input", columnDisplayClass], flex: 1 },
            {
              default: () => [
                h(
                  "input",
  ...
  genLabel(label: string|VNode[]): VNode {
    const limitLabelWidth = this.isColumnDisplay ? "column-flex-item" : "limit-label-width";
    return h(VFlexItem, { class: ["v-field-label", limitLabelWidth] }, {
      default: () => [
  ...
  render() {
    ...
    return h("div", { class: ["v-field", "border-top-bottom", this.isColumnDisplay ? "v-field-column" : ""] }, {
      default: () => [
        h(VFlex,
          {
            flexDirection: this.display,
            class: ["v-field-container"],
          },
          {
